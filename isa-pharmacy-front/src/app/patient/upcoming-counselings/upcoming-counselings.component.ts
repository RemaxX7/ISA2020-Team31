...
export class UpcomingCounselingsComponent implements OnInit {
  ...
  constructor(private counselingService: CounselingService, private toastr: ToastrService) { }
  ...
  Cancel(counseling: any) {
    this.counselingService.cancel(counseling.id).subscribe(data => {
      this.toastr.success('Counseling cancelled.');
      this.GetCounselings(this.currentPage, this.selected);
    },
    err => {
